...
output_dir = r"D:\AI4Charting\大作业\processed2"
...
X = np.array([data[i] for i in sorted_idx])         # shape: (num_samples, 12, 4)
...
data_date = np.array([data_date[i] for i in sorted_idx])# shape: (num_samples,)
returns = np.array([returns[i] for i in sorted_idx])  # shape: (num_samples,)
print(y.mean())
print(X.shape, y.shape, dtype.shape, data_date.shape, returns.shape)
# 保存到 .npy 文件
np.save(os.path.join(output_dir, "X.npy"), X)
np.save(os.path.join(output_dir, "y.npy"), y)
np.save(os.path.join(output_dir, "data_type.npy"), dtype)
np.save(os.path.join(output_dir, "data_date.npy"), data_date)
