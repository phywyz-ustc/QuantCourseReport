...
# 读取数据
X = np.load('D:\AI4Charting\大作业\processed\X.npy')         # shape: (N, 12, 5)
X[..., -1] = np.log1p(X[..., -1])  # 对最后一列进行对数变换
y = np.load('D:\AI4Charting\大作业\processed\y.npy')         # shape: (N,)
data_type = np.load('D:\AI4Charting\大作业\processed\data_type.npy')  # shape: (N,)

# 划分训练和测试
X_train, y_train = X[data_type == 0], y[data_type == 0]
X_test, y_test = X[data_type == 1], y[data_type == 1]
...
class LSTMModel(nn.Module):
    def __init__(self, input_dim=5, hidden_dim=64, num_layers=2):
        ...
        self.fc = nn.Linear(hidden_dim, 1)  # 回归输出
